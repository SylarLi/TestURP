...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public PostProcessPass(RenderPassEvent evt, PostProcessData data, Material blitMaterial = null)
        {
            ...
            if (data.TransparentOverlay && 
                SystemInfo.IsFormatSupported(GraphicsFormat.R16G16B16A16_SFloat, FormatUsage.Linear | FormatUsage.Render))
            {
                m_DefaultHDRFormat = GraphicsFormat.R16G16B16A16_SFloat;
                m_UseRGBM = false;
                m_Transparent = true;
            }
            else if (SystemInfo.IsFormatSupported(GraphicsFormat.B10G11R11_UFloatPack32, FormatUsage.Linear | FormatUsage.Render))
            {
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            {
                // Reset uber keywords
                var uber = m_Transparent ? m_Materials.uberT : m_Materials.uber;
                ...
                if (bloomActive)
                {
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.Bloom)))
                        SetupBloom(cmd, GetSource(), uber);
                }

                // Setup other effects constants
                SetupLensDistortion(uber, isSceneViewCamera);
                SetupChromaticAberration(uber);
                SetupVignette(uber);
                SetupColorGrading(cmd, ref renderingData, uber);

                // Only apply dithering & grain if there isn't a final pass.
                SetupGrain(cameraData, uber);
                SetupDithering(cameraData, uber);

                if (RequireSRGBConversionBlitToBackBuffer(cameraData) && m_EnableSRGBConversionIfNeeded)
                    uber.EnableKeyword(ShaderKeywordStrings.LinearToSRGBConversion);
                ...
                if (m_IsStereo)
                {
                    Blit(cmd, GetSource(), BuiltinRenderTextureType.CurrentActive, uber);
                ...
                else
                {
                    ...
                    cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, uber);
        ...
        void SetupBloom(CommandBuffer cmd, int source, Material uberMaterial)
        {
            ...
            var bloomMaterial = m_Transparent ? m_Materials.bloomT : m_Materials.bloom;
        ...
        class MaterialLibrary
        {
            ...
            public readonly Material bloomT;
            public readonly Material uber;
            public readonly Material uberT;
            ...
            public MaterialLibrary(PostProcessData data)
            {
                ...
                bloomT = Load(data.shaders.bloomTPS);
                uber = Load(data.shaders.uberPostPS);
                uberT = Load(data.shaders.uberPostTPS);
            ...
            internal void Cleanup()
            {
                ...
                CoreUtils.Destroy(bloomT);
                CoreUtils.Destroy(uber);
                CoreUtils.Destroy(uberT);

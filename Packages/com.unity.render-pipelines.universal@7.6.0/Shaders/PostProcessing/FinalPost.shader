Shader "Hidden/Universal Render Pipeline/FinalPost"
{
    HLSLINCLUDE
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            half4 pixel = Load(positionSS, 0, 0);
            half3 color = pixel.xyz;

            #if _FXAA
            {
                // Edge detection
                half3 rgbNW = Load(positionSS, -1, -1).xyz;
                half3 rgbNE = Load(positionSS,  1, -1).xyz;
                half3 rgbSW = Load(positionSS, -1,  1).xyz;
                half3 rgbSE = Load(positionSS,  1,  1).xyz;
